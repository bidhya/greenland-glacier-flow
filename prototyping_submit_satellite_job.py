...
def get_working_directory(execution_mode, base_dir):
    """Get appropriate working directory based on execution mode"""
    if execution_mode == 'hpc':
        return '$TMPDIR'  # Use SLURM temporary directory
    else:
        # For local execution, use a subdirectory of base_dir
        local_work_dir = Path(base_dir) / 'work'
...
def execute_locally(satellite, regions, date1, date2, base_dir, log_name, **kwargs):
    ...
    scripts_source = script_dir / '1_download_merge_and_clip'
    scripts_dest = work_dir / '1_download_merge_and_clip'
    
    print(f"Copying scripts from {scripts_source} to {scripts_dest}")
    ...
    os.chdir(scripts_dest)
    
    try:
        # Build command based on satellite type
        if satellite.lower() == "sentinel2":
            cmd = [
                ...
                '--base_dir', f"{base_dir}/1_download_merge_and_clip/sentinel2",
                '--log_name', log_name
            ]
        ...
        # Execute the command with conda environment activation
        conda_cmd = f"conda run -n glacier_velocity {' '.join(cmd)}"
        print(f"Full command with conda: {conda_cmd}")
        
        result = subprocess.run(conda_cmd, shell=True, check=True)
        
        print("=" * 60)
        print(f"LOCAL EXECUTION: {satellite} processing completed successfully")
        print(f"Check outputs at: {base_dir}")
        
    except subprocess.CalledProcessError as e:
        print(f"ERROR: Local execution failed with return code {e.returncode}")
        raise
    except Exception as e:
        print(f"ERROR: Local execution failed: {e}")
        raise
    finally:
    ...
    if execution_mode == 'local':
        ...
        if dry_run:
            ...
            execute_locally(
                ...
                post_processing_flag=post_processing_flag,
